 ...
 * @brief random grid data generator for drilling problem
 */
...
vector<vector<int> > randSymmMatrixGen(int holes, int size, int divisions) {
  ...
  for (int i = 0; i < holes; i++)
    for (int j = 0; j < i; j++) {
      int xSquaredDiff = pow(xPositions[i] - xPositions[j], 2);
      int ySquaredDiff = pow(yPositions[i] - yPositions[j], 2);
      distancesMatrix[i][j] = distancesMatrix[j][i] =
            sqrt(xSquaredDiff + ySquaredDiff);
    }
